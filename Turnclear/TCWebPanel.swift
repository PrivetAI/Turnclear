...
///
/// `TCPanelSession` and `TCPanelCookies` below are what keep a sign-in alive across a cold start:
/// the panel resumes the page the user was really on, and the cookie jar is mirrored out and back.
struct TCWebPanel: UIViewRepresentable {
    let address: String
    /// Our own host — the tracker hop, which must never be remembered as a resume point. The
    /// Privacy Policy sheet passes none, which also switches remembering off for it.
    var trackerHost: String = ""
    /// Where to go if `address` is a resumed address that has since gone dead. nil when the panel
    /// already started at the tracker link (and for the Privacy Policy sheet).
    ...
    /// Called when nothing loads at all — live or cached. The caller shows the native app.
    ...
    final class Coordinator: NSObject, WKNavigationDelegate {
        ...
        /// What the panel was asked to load first — the cache candidate when it was a resumed
        /// address.
        var initialAddress = ""
        ...
        /// navigation delegate callback, so `didCommit` never sees it and the resume address would
        /// be stuck on whatever loaded last. `url` is KVO-compliant and moves for both, and
        /// `remember` is idempotent, so this simply covers more.
        func watchAddress(of panel: WKWebView) {
            ...
            addressWatch = panel.observe(\.url, options: [.new]) { [weak self] observed, _ in
                ...
                TCPanelSession.remember(observed.url, trackerHost: self.trackerHost)
            }
        }
        ...
        func webView(_ webView: WKWebView, didCommit navigation: WKNavigation!) {
            TCPanelSession.remember(webView.url, trackerHost: trackerHost)
            release()
        }

        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            TCPanelSession.remember(webView.url, trackerHost: trackerHost)
            // The jar is at its most interesting the moment a page settles: a sign-in POST has
            // landed by now.
        ...
        func webView(_ webView: WKWebView,
            ...
            // After the first paint a failed navigation is an ordinary failed navigation inside a
            // working session: WebKit shows its error and the user can go back. Recovery is only
            // for a panel that never got off the ground — otherwise this would yank a browsing
            // user into the native app.
            guard !alreadyFired else { return }
            recover(webView)
        }

        /// resumed address live -> tracker link live -> the same address from the on-disk cache
        /// -> give up and hand back the native app. Never leave anyone on WebKit's error page:
        /// full screen, no back, no reload, nothing to act on.
        private func recover(_ webView: WKWebView) {
            // 1. The resumed address is dead. Stop resuming it and re-enter via the tracker.
            if !triedFallback, let fallback = fallbackAddress, let target = URL(string: fallback) {
                triedFallback = true
                TCPanelSession.forget()
                webView.load(URLRequest(url: target))
                return
            }
            // 2. Nothing loads live. A stale page from disk still shows the user their account;
            //    an error page shows them nothing they can act on. Only worth trying for a real
            //    page — the tracker link is a 302 with nothing cached.
            if !triedCache, fallbackAddress != nil, let target = URL(string: initialAddress) {
                triedCache = true
                webView.load(URLRequest(url: target,
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        context.coordinator.initialAddress = address
        ...
        // Cookies FIRST, then load. The other order signs the user out on every cold start, and
        // the splash is still up so the wait is invisible.
        let target = address
        TCPanelCookies.restore { [weak panel] in
    ...
    // callbacks is the only thing that belongs here.
...
/// re-running the redirect chain from the top. It decides only WHAT the panel loads once the gate
/// has already said yes — never whether it opens.
enum TCPanelSession {
    private static let addressKey = "tc.panel.resume.address"
    private static let stampKey = "tc.panel.resume.stamp"
    ...
    static func remember(_ url: URL?, trackerHost: String) {
        // No tracker host means this is not the launch panel — the Privacy Policy sheet passes
        // none, which switches remembering off for it. Without this guard the next launch would
        // resume the privacy page instead of the offer.
...
/// Mirrors the WebKit cookie jar out to `UserDefaults` and back. A cookie with no expiry — a plain
/// session cookie, which is what most sign-ins hand out — lives only in WebKit's networking
/// process and dies with it; re-injecting it with an explicit expiry is what keeps the sign-in.
enum TCPanelCookies {
    private static let key = "tc.panel.cookies"
    ...
    /// WebKit has been seen to swallow a `setCookie` completion. Never let that hold a launch:
    /// past this the page loads regardless.
    ...
    static func snapshot() {
        WKWebsiteDataStore.default().httpCookieStore.getAllCookies { cookies in
            ...
            // Wholesale overwrite, so a sign-out that empties the jar empties the mirror too and
            // cannot resurrect a dead session on the next launch.
            UserDefaults.standard.set(payload, forKey: key)
        }
    }

    /// Re-injects the mirror and then calls back. The caller MUST wait for this before the first
    /// load: a request that goes out early is the one that arrives signed out.
